...
public class IRNPRequestErrorWithdrawMessage extends IRNPMessage {
	...
	public IRNPRequestErrorWithdrawMessage() {
		vers = 0x01;
		flags = 0x00;
		msgType = IRNPMessageTypes.MESSAGE_REPLY_WITHDRAW_ERROR;
		length = IRNPMessageTypes.IRNP_MESSAGE_HEADER_LENGTH;
	}

	@Override
	public void encode() throws IRNPProtocolViolationException {
		if(senderTemplate == null || receiverTemplate == null || asPath == null || errorSpec == null) {
			System.out.println("IRNP Request Error Withdraw Message misses parts, cannot be encoded");
			return;
		}
		length = length + senderTemplate.getLength() + receiverTemplate.getLength() + asPath.getLength() + errorSpec.getLength();
		...
		System.out.println("IRNP Request Error Withdraw Message encoding accomplished");
	}
