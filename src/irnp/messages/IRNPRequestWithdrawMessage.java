...
public class IRNPRequestWithdrawMessage extends IRNPMessage {
	
	protected SenderTemplate senderTemplate;
	protected ReceiverTemplate receiverTemplate;
	protected ASPath asPath;
	
	public IRNPRequestWithdrawMessage() {
		vers = 0x01;
		flags = 0x00;
		msgType = IRNPMessageTypes.MESSAGE_REQUEST_WITHDRAW;
		length = IRNPMessageTypes.IRNP_MESSAGE_HEADER_LENGTH;
	}

	@Override
	public void encode() throws IRNPProtocolViolationException {
		if(senderTemplate == null || receiverTemplate == null || asPath == null) {
			System.out.println("IRNP Request Withdraw Message misses parts, cannot be encoded");
			return;
		}
		length = length + senderTemplate.getLength() + receiverTemplate.getLength() + asPath.getLength();
		...
		System.out.println("IRNP Request Withdraw Message encoding accomplished");
	}
